...
;      ;; this publishes on executor-receive-queue-map (read by executors, I assume)
;      :transfer-local-fn (mk-transfer-local-fn <>)
;      :receiver-thread-count (get storm-conf WORKER-RECEIVER-THREAD-COUNT)
;      ;; this publishes on transfer-queue (sent along)
...
;
;        ;; refresh connections to tasks that accept input from this worker
...
;
;        ;; launch a receive thread, name stupid because returns shutdown fn
;        receive-thread-shutdown (launch-receive-thread worker)
;
;        ;; handler for transfering tuples further along
;        transfer-tuples (mk-transfer-tuples-handler worker)
;
;        ;; this is the thread that runs the handler
...
;; TODO: use this if wanna make a topology active/non-active
;(defn refresh-storm-active
;  ([worker]
;    (refresh-storm-active worker (fn [& ignored] (schedule (:refresh-active-timer worker) 0 (partial refresh-storm-active worker)))))
;  ([worker callback]
